...
│  │  编译阶段    │                                                           │
│  │             │                                                           │
│  │ - 编译源码  │ ──► compile ──► compilation ──► make ──► finishModules │
│  │ - 分析依赖  │     (创建 Compilation)  (构建依赖图)  (依赖图完成)        │
│  └─────────────┘                                                           │
│        │                                                                  │
│        ▼                                                                  │
│  ┌─────────────┐                                                           │
...
**重要说明**：
- `compilation` 钩子在 `make` 之前触发
- `compilation` 阶段模块数量为 0（依赖图尚未生成）
- `make` 钩子触发后开始构建模块依赖图
- `finishModules` 钩子触发时所有模块依赖图构建完成
...
| 执行顺序 | 钩子 | 类型 | 说明 |
|---------|------|------|------|
| 1 | `beforeRun` | AsyncSeriesHook | 开始运行前 |
| 2 | `run` | AsyncSeriesHook | 开始编译 |
| 3 | `beforeCompile` | SyncBailHook | 编译前准备 |
| 4 | `compile` | SyncHook | 开始编译 |
| 5 | `compilation` | SyncHook | 创建 Compilation 对象（此时模块数量为 0） |
| 6 | `make` | AsyncParallelHook | 开始构建模块依赖图 |
| 7 | `finishModules` | AsyncSeriesHook | 所有模块依赖图构建完成 |
| 8 | `emit` | AsyncSeriesHook | 输出资源前 |
